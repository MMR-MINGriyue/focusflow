...
// 创建Store
export const useUnifiedTimerStoreOptimized = create<UnifiedTimerStore>()(
...
export const timerSelectors = {
  // 基础状态选择器
  useTimerState: () => useUnifiedTimerStoreOptimized(
    (state) => ({
  ...
  // 设置选择器
  useTimerSettings: () => useUnifiedTimerStoreOptimized(
    (state) => state.settings
  ),

  // 今日统计选择器
  useTodayStats: () => useUnifiedTimerStoreOptimized(
    (state) => state.todayStats
  ),

  // 效率评分相关选择器
  useEfficiencyRating: () => useUnifiedTimerStoreOptimized(
    (state) => ({
  ...
  // 控制方法选择器
  useTimerControls: () => useUnifiedTimerStoreOptimized(
    (state) => ({
      start: state.start,
      pause: state.pause,
      reset: state.reset,
  ...
  // 完整状态选择器（用于需要所有状态的组件）
  useFullTimerState: () => useUnifiedTimerStoreOptimized(
    (state) => state
  ),
};
